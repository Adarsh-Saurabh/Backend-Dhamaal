...
def fetching_available_teacher_as_per_location(request: HttpResponse):
    subject = request.POST['subject']
    username = request.session.get('username')
    #fetching student zipcode
    all_student = Students.objects.all()
    for student in all_student:
        if student.username == username:
            current_student_zipcode = student.zipcode
            break;
    all_teachers = Teachers.objects.all()
    list_of_available_teachers = []
    for teacher in all_teachers:
        if teacher.subject == subject and teacher.zipcode == current_student_zipcode:
...
def generate_OTPs(request, email_id):
    ...
    server_for_email.sendmail('dadupandey27@gmail.com',request.session.get('email'),str(temp_otp))
...
def validator2(request: HttpResponse):
    ...
    if (request.POST['email_otp'] == str(request.session.get('email_OTP'))) and (request.POST['phone_otp'] == str(request.session.get('phoneOTP'))):
        student = Students(random.randint(1,10000000000),request.session.get('firstname'), request.session.get('lastname'), request.session.get('username'), request.session.get('gender'), request.session.get('email'),request.session.get('contactno'), request.session.get('password'), request.session.get('zipcode'))
        student.save()
        context = {
            "msg":"You have been registered successfully."
        }
...
def student_teacher_connection(request: HttpResponse):
    ...
    if not subscription:
        return render(request, 'sprice.html')
    else:
